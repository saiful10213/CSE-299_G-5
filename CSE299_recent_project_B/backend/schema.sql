...
-- NID applications (submitted by users)
...
-- Birth certificate applications (submitted by users)
...
-- Final Approved Birth Certificate (resolved 'union_name' issue)
...
-- Voting records (one vote per user)
CREATE TABLE votes (
  id INT AUTO_INCREMENT PRIMARY KEY,
  user_id INT NOT NULL,
  ...
  UNIQUE KEY (user_id)  -- Prevent multiple votes by the same user
);
